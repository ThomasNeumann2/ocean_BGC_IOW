...
!   call g_tracer_add_param('o2_2_nfix', cobalt%o2_2_nfix, (118.0+3.0/(5.0+3.0)*(150.0-118.0))/16.0) ! mol O2 mol N-1
    ...
    call g_tracer_add_param('bresp_Lg', phyto(LARGE)%bresp,0.028/sperd)                  ! sec-1 
    ...
    call g_tracer_add_param('ml_aclm_efold', cobalt%ml_aclm_efold, 3.0)                ! dimensionless
    call g_tracer_add_param('irrad_day_thresh', cobalt%irrad_day_thresh, 0.0 )           ! watts m-2 
    ...
    call g_tracer_add_param('phi_ldon_mdz',zoo(2)%phi_ldon, 0.7*0.15)      ! dimensionless
    ...
    call g_tracer_add_param('phi_ldop_mdz',zoo(2)%phi_ldop, 0.65*0.15)     ! dimensionless
    ...
    call g_tracer_add_param('phi_srdon_mdz',zoo(2)%phi_srdon, 0.1*0.15)    ! dimensionless
    ...
    call g_tracer_add_param('phi_srdop_mdz',zoo(2)%phi_srdop, 0.15*0.15)   ! dimensionless
    ...
    call g_tracer_add_param('phi_sldon_mdz',zoo(2)%phi_sldon, 0.2*0.15)    ! dimensionless
    ...
    call g_tracer_add_param('phi_sldop_mdz',zoo(2)%phi_sldop, 0.2*0.15)    ! dimensionless
...
!            implementation='duce',   &
  ...
  subroutine generic_COBALT_update_from_source(tracer_list,Temp,Salt,rho_dzt,dzt,hblt_depth,&
    ...
    real :: log_irrad_aclm_bot, log_irrad_aclm_top, log_irrad_aclm_slope 
    real :: frac_sfc_irrad_aclm, irrad_aclm_thresh, frac_thickness
...
!
    ...
    do j = jsc, jec ; do i = isc, iec   !{
       ...
       !write(outunit,*) 'f_irr_aclm_sfc (pre) = ',cobalt%f_irr_aclm_sfc(i,j,1)
       if ((sfc_irrad(i,j).ge.cobalt%irrad_day_thresh) .or. &
           (cobalt%f_irr_aclm_sfc(i,j,1).lt.cobalt%irrad_day_thresh)) then
         cobalt%f_irr_aclm_sfc(i,j,1) = (cobalt%f_irr_aclm_sfc(i,j,1) + (sfc_irrad(i,j) - &
           cobalt%f_irr_aclm_sfc(i,j,1)) * min(1.0,cobalt%gamma_irr_aclm * dt)) * grid_tmask(i,j,1)
       endif
       !write(outunit,*) 'f_irr_aclm_sfc (post) = ',cobalt%f_irr_aclm_sfc(i,j,1)
       ...
       log_irrad_aclm_top = log(max(cobalt%f_irr_aclm_sfc(i,j,1),epsln))
       do k = 1, nk !{
          tmp_irrad = 0.0
          do nb=1,nbands !{
          ...
          cobalt%irr_aclm_inst(i,j,k) = tmp_irrad * grid_tmask(i,j,k)
         
          ! integrated daytime irradiance over photoacclimation timescale maintaining z-resolution within 
          ! the mixed layer, used for calculating photoacclimation irradiance in deep mixed layers
          if ((sfc_irrad(i,j).ge.cobalt%irrad_day_thresh) .or. &
              (cobalt%f_irr_aclm_sfc(i,j,1).lt.cobalt%irrad_day_thresh) ) then
                cobalt%f_irr_aclm_z(i,j,k) = (cobalt%f_irr_aclm_z(i,j,k) + (cobalt%irr_inst(i,j,k) - &
          ...
          if ( (k == 1) .or. (tmp_hblt .lt. hblt_depth(i,j)) ) then !{
             ...
             ! estimate the acclimation irradiance at the bottom of the cell.
             ! if the acclimation irradiance at the bottom of the cell is below the threshold, calculate
             ! partial cell based on when the threshold is met.  This is an approximation that only
             ! partly accounts for subgrid scale changes in light. However, it ensures the acclimation
             ! parameterization is not overly sensitive to thick cells at the base of the mixed layer
             ! NOTE: bottom cell uses the irradiance "slope" from the two cells above to estimate bottom irrad
             if (k.lt.nk) then
                log_irrad_aclm_slope = (log(max(cobalt%f_irr_aclm_z(i,j,k+1),epsln))-log(max(cobalt%f_irr_aclm_z(i,j,k),epsln)))/ &
                                       (zmid(i,j,k+1)-zmid(i,j,k))
             endif
             log_irrad_aclm_bot = log(max(cobalt%f_irr_aclm_z(i,j,k),epsln))+log_irrad_aclm_slope*(zbot(i,j,k)-zmid(i,j,k))
             if (exp(log_irrad_aclm_bot).ge.irrad_aclm_thresh) then
                tmp_irrad_aclm = tmp_irrad_aclm + cobalt%irr_inst(i,j,k) * dzt(i,j,k)
                tmp_zaclm = tmp_zaclm + dzt(i,j,k)
             elseif ((exp(log_irrad_aclm_top).ge.irrad_aclm_thresh).and.(exp(log_irrad_aclm_bot).lt.irrad_aclm_thresh)) then 
                frac_thickness = (log(max(irrad_aclm_thresh,epsln))-log_irrad_aclm_bot)/(log_irrad_aclm_top-log_irrad_aclm_bot)
                tmp_irrad_aclm = tmp_irrad_aclm + cobalt%irr_inst(i,j,k)*frac_thickness*dzt(i,j,k)
                tmp_zaclm = tmp_zaclm + frac_thickness*dzt(i,j,k)
             endif
             log_irrad_aclm_top = log_irrad_aclm_bot
          endif
       ...
       ! timescale (see below)
       cobalt%irr_aclm_inst(i,j,1:kblt(i,j)) = tmp_irrad_aclm/max(1.0e-6,tmp_zaclm)
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
      ...
      ! calculate temperature dependence for all phytoplankton
       ...
       do n = 1, NUM_PHYTO   !{
          ...
          phyto(n)%theta(i,j,k) = max(phyto(n)%thetamax / (1.0 +            &
            phyto(n)%thetamax*phyto(n)%alpha*cobalt%f_irr_aclm(i,j,k)*0.5 /  &
            P_C_m),cobalt%thetamin)
    !      theta_temp = (phyto(n)%thetamax - cobalt%thetamin)/2.0
    !      theta_step = (phyto(n)%thetamax - cobalt%thetamin)/4.0
    !      a_theta = P_C_m/(phyto(n)%alpha*max(cobalt%f_irr_aclm(i,j,k),epsln)) 
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
       do n = 1, NUM_PHYTO  !{
          if (phyto(n)%q_fe_2_n(i,j,k).lt.phyto(n)%fe_2_n_max) then
             ...
             phyto(n)%jexuloss_fe(i,j,k) = cobalt%expkT(i,j,k)*phyto(n)%bresp*phyto(n)%f_fe(i,j,k)
          endif
       enddo   !} n
    enddo; enddo ; enddo !} i,j,k
    !
